...
final class Log4j2LoggerBackend extends LoggerBackend {
  ...
  private void forceLog(Logger logger, Log4j2SimpleLogEvent logEntry) {
    // Logger#callAppenders circumvents any evaluation of whether to log or not to log.
    LoggerConfig loggerConfig = new LoggerConfig(logger.toString(), null, false);
    loggerConfig.log(logEntry.asLoggingEvent());
  }
}
